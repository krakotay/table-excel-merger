...
async def process_messages(messages: List[types.Message]):
    ...
    for msg in messages:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".xlsx", delete=False, dir="temp"
        ) as temp_file:
    ...
    df = merge_excel(
        temp_files_paths[0]["path"],
        temp_files_paths[1]["path"] if len(temp_files_paths) > 1 else None,
    )
    with tempfile.NamedTemporaryFile(
        suffix=".xlsx", 
        prefix=f"{(messages[0].document.file_name.removesuffix('.xlsx') + '_' + messages[1].document.file_name.removesuffix('.xlsx') if len(messages) > 1 else messages[0].document.file_name.removesuffix('.xlsx'))}_", 
        delete=False, dir="temp"
    ) as temp_file:
        temp_file_path = temp_file.name
    ...
    await messages[0].answer(
        f"Поиск ФИО, пожалуйста, подождите... Ожидаемое время: {wait_time_s} секунд"
    )
    ...
    with tempfile.NamedTemporaryFile(
        ...
        try:
            ...
            for i, row in enumerate(ws.iter_rows(), 1):
                if (
                    i > 1
                ):  # Пропускаем первую строку (заголовки), так как уже обработали
        ...
        await messages[0].answer_document(FSInputFile(path=temp_file_path))
    await messages[0].answer(
        f"Файлы '{temp_files_paths[0]['name']}' '{temp_files_paths[1]['name'] if len(temp_files_paths) > 1 else ''}' успешно скачаны и обработаны."
    )
...
async def excel_handler(messages: List[types.Message]):
    ...
    if len(messages) != 2:
        await messages[0].answer(
            "Пожалуйста, отправьте ровно два файла Excel одним сообщением (как группу)."
        )
        return
    ...
    try:
        ...
        logging.error(
            f"Ошибка при обработке группы {media_group_id}: {e}", exc_info=True
        )
        await messages[0].answer(
            f"Произошла ошибка при скачивании или обработке файлов: {e}"
        )
